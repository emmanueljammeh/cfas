...
def extract_cluster0_data():
    from sklearn.preprocessing import LabelEncoder
    mkdir('Analysis')
    mkdir('Analysis/data')

    fName='G:/My Drive/ejammeh/Projects/Emmanuel/CFAS/Mohammed/feature_selection/data/clinical_data_balanced_original_cleaned.csv'
    df=load_dataset(fName)#.dropna()
    ...
    df0['membership']=df0.membership.replace({0:5});
    df0['membership']=df0.membership.replace({1:0, 2:0, 5:1});

    ''' save data '''
    df0.to_csv('Analysis/data/cluster_0_dataset.csv',index=False)

    ''' Plot membership '''
    sns.set(style="ticks")
    sns.countplot(x="membership", data=df0, palette="bwr")
    #plt.show()
    plt.title('Cluster 0 membership')
    plt.ylabel('Count')
    plt.xlabel('Membership')
    plt.tight_layout()
    plt.savefig('Analysis/data/cluster_0_membership.png')
    plt.savefig('Analysis/data/cluster_0_membership.pdf')

    # create data and labels
    df0_data, df0_labels=df0.drop(columns=['membership']),df0['membership']
    
    return df0_data, df0_labels

def extract_cluster1_data():
    from sklearn.preprocessing import LabelEncoder
    mkdir('Analysis')
    mkdir('Analysis/data')
    ...
    ''' Create a dataset of cluster 0 vs rest '''
    df1['membership']=df1.membership.replace({0:0, 1:1, 2:0});

    ''' save data '''
    df1.to_csv('Analysis/data/cluster_1_dataset.csv',index=False)

    ''' Plot membership '''
    sns.set(style="ticks")
    sns.countplot(x="membership", data=df1, palette="bwr")
    #plt.show()
    plt.title('Cluster 1 membership')
    plt.ylabel('Count')
    plt.xlabel('Membership')
    plt.tight_layout()
    plt.savefig('Analysis/data/cluster_1_membership.png')
    plt.savefig('Analysis/data/cluster_1_membership.pdf')

    # create data and labels
    df1_data, df1_labels=df1.drop(columns=['membership']),df1['membership']

    return df1_data, df1_labels


def extract_cluster2_data():
    from sklearn.preprocessing import LabelEncoder
    mkdir('Analysis')
    mkdir('Analysis/data')
    ...
    ''' Create dataset for cluster 2 '''
    df2=df

    ''' Create a dataset of cluster 0 vs rest '''
    df2['membership']=df2.membership.replace({0:0, 1:0, 2:1});

    ''' save data '''
    df2.to_csv('Analysis/data/cluster_2_dataset.csv',index=False)

    ''' Plot membership '''
    sns.set(style="ticks")
    sns.countplot(x="membership", data=df2, palette="bwr")
    #plt.show()
    plt.title('Cluster 2 membership')
    plt.ylabel('Count')
    plt.xlabel('Membership')
    plt.tight_layout()
    plt.savefig('Analysis/data/cluster_2_membership.png')
    plt.savefig('Analysis/data/cluster_2_membership.pdf')

    # create data and labels
    df2_data, df2_labels=df2.drop(columns=['membership']),df2['membership']

    return df2_data, df2_labels
...
def boruta_feature_ranking(data, labels):
    ...
    raw_ranking=pd.DataFrame({'feature':data.columns,'rank':boruta_feature_selector.ranking_})
    ranking=pd.DataFrame({'feature':data.columns,'rank':boruta_feature_selector.ranking_}).sort_values(by='rank', ascending=True)
    
    selected=pd.DataFrame({'feature':final_features})
    print(selected)
    
    #boruta_ranking['rank'].sum()
    ranking['normalised_rank']=ranking['rank']/ranking['rank'].sum().round(3)
...
def shap_feature_ranking(data,target):
    ...
    shap_ranking['normalised_rank']=abs(shap_ranking['importance'])/abs(shap_ranking.importance).sum().round(3)
    
    
    unranked_shap_ranking=pd.DataFrame(pd.DataFrame(shap_values, columns=data.columns).sum(),columns=['importance'])
    unranked_shap_ranking['normalised_rank']=abs(unranked_shap_ranking['importance'])/abs(unranked_shap_ranking.importance).sum().round(3)
