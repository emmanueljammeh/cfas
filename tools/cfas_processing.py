...
'''
    1. A method that removes features with very similar values. For example,  a feature that contains 0 for every instance.
    2. A filter method that removes correlated features based on Pearson or  Spearman's coefficient.
    3. A recursive feature elimination algorithm with cross validation.
    4. The Boruta method which uncovers features that are relevant to the outcome.
...
class FeatureSelector:
    ...
    Citation
    NB: adapted from Frank Ceballos
    ...
    # Fit method::  Calls the feature selection methods in the order specified in steps and  determines the selected features. 
